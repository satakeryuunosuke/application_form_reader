...
for ch_idx in range(10):
    ...
    for char, pat in CODE39_PATTERNS.items():
        ...
        # wide平均はnarrow平均よりも明らかに大きくなければならない
        if avg_w <= avg_n * 1.25:
            continue
        # 分散コスト: 各エレメントが自グループ平均からどれだけ揃っているか
        cost = sum((x - avg_n)**2 for x in n_lens) + sum((x - avg_w)**2 for x in w_lens)
        scores.append((cost, char, avg_w, avg_n))
    scores.sort(key=lambda s: s[0])
    if scores:
        best = scores[0]
        decoded.append(best[1])
        print(f"Char {ch_idx}: best='{best[1]}' (cost={best[0]:.1f}, w={best[2]:.1f}, n={best[3]:.1f})")
    else:
        decoded.append('?')
        print(f"Char {ch_idx}: NO CANDIDATE")
    c_idx += 10

print("===> RESULT:", ''.join(decoded))
