...
for th_val in range(110, 180, 5):
    ...
    for start_i in range(len(runs) - 98):
        ...
        for ch_num in range(10):
            ...
            # 9エレメント中、黒バーは 0, 2, 4, 6, 8 (5本)
            # スペースは 1, 3, 5, 7 (4本)
            bars = [elems[0], elems[2], elems[4], elems[6], elems[8]]
            spaces = [elems[1], elems[3], elems[5], elems[7]]
            
            # 各Code 39パターンとの類似度（マッチング距離）を計算
            best_char = None
            best_dist = float('inf')
            ...
            for char, pat in CODE39_PATTERNS.items():
                ...
                if avg_w <= avg_n:
                    continue
                # コスト: narrowがavg_nから離れている二乗誤差 + wideがavg_wから離れている二乗誤差
        ...
        if len(decoded) == 10:
            ...
            if res_text.startswith('*') and res_text.endswith('*'):
                print(f"MATCH with th={th_val}, start_i={start_i}: {res_text}")
